...
/*
First idea
If we assume only left we can check the height of the left branches

if is a right then the value is the sum of left height
if left then 1 plus the left height
if root = 0

result

After checking the test the previous assumption was wrong, it's just the normal height
*/
...
1        3    10
  ...
  */
    int leftHeight(Node node) {
        ...
        int leftHeight = leftHeight(node.left) + 1;

        int rightHeight = leftHeight(node.right);
        if (rightHeight > 0) {
            rightHeight++;
        }
