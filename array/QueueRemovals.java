 ...
 */
/*
Extract to a list, keep the max number, remove, and then put back to the queue
keep original position in a queue also
*/

class QueueRemovals {


    private int extractPosition(Queue<Integer> elements, Queue<Integer> positions, int x) {
        if (elements.isEmpty()) {
            return 0;//We can throw some custom exceptions
        }
        int pos = 0;
        ...
        int max = 0;
        int maxPosition = 0;
        while (pos < x && !elements.isEmpty()) {//run until x or the queue is empty
            int element = elements.poll();
            currentPositions.add(positions.poll());
            if (element > max) {//check for max in the extracted values
                max = element;
                maxPosition = pos;
            }
            currentElements.add(element - 1);//save the decreased value into the list
            pos++;
        }
        currentElements.remove(maxPosition);
        int result = currentPositions.get(maxPosition);
        currentPositions.remove(maxPosition);
    ...
    int[] findPositions(int[] arr, int x) {
        ...
        for (int i = 0; i < x; i++) {//x iterations
            result[i] = extractPosition(elements, positions, x);
        }
